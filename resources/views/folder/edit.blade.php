@extends('layouts.base', ['title' => 'Edit Folder'])
@section('content')
        ...
        <div class="col-6">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h6 class="text-primary">Edit Folder</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('folder.update', $folder->id) . '?f=' . $folder->folder_id ?? '' }}" method="post">
                        @csrf
                        @method('patch')
                        <div class="form-group">
                            <label for="nama">Nama Folder</label>
                            <input type="text" class="form-control" name="nama" id="nama" placeholder="Berikan nama folder" value="{{ old('nama') ?? $folder->nama }}">
                            @error('nama')
